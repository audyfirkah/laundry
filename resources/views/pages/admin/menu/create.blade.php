...
@section('content')
    <div class="container mx-auto px-6 py-10">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-semibold text-gray-800">Tambah Menu</h2>
            ...
            <form action="{{ route('admin.menu.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="nama_barang" id="nama_barang" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500">
                            ...
                            <option value="baju" {{ old('nama_barang') == 'baju' ? 'selected' : '' }}>Baju</option>
                            <option value="karpet" {{ old('nama_barang') == 'karpet' ? 'selected' : '' }}>Karpet</option>
                            <option value="boneka" {{ old('nama_barang') == 'boneka' ? 'selected' : '' }}>Boneka</option>
                    </select>
                    @error('nama_barang')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="kategori" id="kategori" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500">
                            ...
                            <option value="dryclean" {{ old('kategori') == 'dryclean' ? 'selected' : '' }}>Dryclean</option>
                    </select>
                    @error('kategori')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="harga" id="harga" value="{{ old('harga') }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500">
                    @error('harga')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
